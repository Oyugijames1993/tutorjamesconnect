...
self.addEventListener('push', (event) => {
  ...
  event.waitUntil((async () => {
    ...
    // Tell open tabs to play custom sound
    ...
    // Show OS notification with sound and vibration
    await self.registration.showNotification(title, {
      body,
      icon:     '/icon-192.png',
      badge:    '/icon-192.png',
      data:     { url, sound_type },
      tag:      sound_type === 'pending' ? 'pending' : url,
      renotify: true,
      vibrate:  [300, 100, 300, 100, 300],
      silent:   false,
      // These trigger the device's default notification sound
      requireInteraction: sound_type === 'pending',
    })
  })())
})
...
self.addEventListener('message', (event) => {
  ...
  if (event.data?.type === 'new-message') {
    unreadCount++
    if (self.navigator?.setAppBadge) self.navigator.setAppBadge(unreadCount)
  }
})
