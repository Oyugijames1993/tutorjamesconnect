...
const C = {
  ...
  text:'#111b21',textMid:'#3b4a54',textSoft:'#667781',textFaint:'#8696a0',
  adminBg:'#e7f8f3',adminText:'#008a6e',providerBg:'#eef0ff',providerText:'#4338ca',clientBg:'#e9f3ff',clientText:'#2a5298',
  green:'#00a884',greenLight:'#e7f8f3',amber:'#d69e2e',amberLight:'#fef6e0',red:'#e53e3e',redLight:'#fff0ef',white:'#ffffff',
}
...
export default function ChatRoom() {
  ...
  const { supported: pushSupported, subscribed: pushSubscribed, enable: enablePush, disable: disablePush } = usePushNotifications()
  ...
  const togglePush = async () => {
    ...
    try {
      ...
      else { const res = await enablePush(); if (!res.ok) setError(res.reason || 'Could not enable notifications.') }
    } finally { setPushBusy(false) }
  }

  const toggleSound = useCallback(() => {
    setSoundEnabled(prev => { const next = !prev; localStorage.setItem('tjc_sound_enabled', String(next)); return next })
  }, [])
  const changeMessageSoundProfile = useCallback((id) => { setMessageSoundProfile(id); localStorage.setItem('tjc_message_sound_profile', id) }, [])
  const changePendingSoundProfile  = useCallback((id) => { setPendingSoundProfile(id);  localStorage.setItem('tjc_pending_sound_profile',  id) }, [])
  ...
  useEffect(() => { if (!isAdmin) return; api.get('/accounts/users/').then(res => { setAvailableProviders(res.data.filter(u => u.role === 'provider')); setAvailableClients(res.data.filter(u => u.role === 'client')) }) }, [])
  ...
  useEffect(() => { if (!activeRoom) return; setUnreadCounts(prev => ({ ...prev, [activeRoom.id]: 0 })) }, [activeRoom?.id])
  ...
  useEffect(() => {
    if (!activeRoom) return
    if (roomWsRefs.current[activeRoom.id]) { roomWsRefs.current[activeRoom.id].disconnect(); delete roomWsRefs.current[activeRoom.id] }
  }, [activeRoom?.id])
  ...
  useEffect(() => {
    ...
    wsRef.current = new ChatWebSocket(activeRoom.id, token, data => {
      ...
      if (data.type === 'presence_snapshot') { setOnlineUserIds(new Set(data.online_user_ids)); return }
      if (data.type === 'presence') { setOnlineUserIds(prev => { const next = new Set(prev); if (data.online) next.add(data.user_id); else next.delete(data.user_id); return next }); return }
      ...
      if (data.type === 'system') { if (!isAdmin && !isProvider) return; setMessages(prev => [...prev, { type: 'system', message: data.message, id: 'sys_' + Date.now() + '_' + Math.random() }]); return }
      if (data.type === 'error') { setError(data.error); setTimeout(() => setError(''), 4000); return }
      if (data.type === 'pending') { setPendingMessages(prev => prev.find(p => p.id === data.id) ? prev : [...prev, data]); if (soundEnabled && isAdmin) playSound('pending', pendingSoundProfile); return }
      if (data.type === 'file:pending') {
        ...
        if (isAdmin || isMine) { setMessages(prev => { const pid = 'file_pending_' + data.id; if (prev.find(m => m.id === pid)) return prev; return [...prev, { type: 'file', id: pid, file_id: data.id, file_name: data.file_name, file_size: data.file_size, file_url: data.file_url ?? null, sender: data.sender, role: data.role, status: 'pending', time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) }] }) }
      }
    })
  ...
  const formatFileSize = n => { if (n < 1024) return `${n} B`; if (n < 1024**2) return `${Math.floor(n/1024)} KB`; return `${(n/1024**2).toFixed(1)} MB` }

  const handleFileUpload = async e => { const file = e.target.files[0]; e.target.value = ''; if (!file) return; await uploadFile(file) }
  ...
  const uploadFile = async file => {
    ...
    try {
      ...
      if (file_id) {
        ...
        setMessages(prev => prev.find(m => m.file_id === file_id) ? prev : [...prev, { type: 'file', id: localId, file_id, file_name: file.name, file_size: formatFileSize(file.size), file_url: (isImageFile(file.name) || isAudioFile(file.name)) ? URL.createObjectURL(file) : null, sender: user?.display_name, role: user?.role, status: fileStatus, time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) }])
      }
    } catch { setError('Failed to upload file.'); setTimeout(() => setError(''), 4000) }
  }

  const startRecording = async () => {
    if (!connected) { setError('Not connected yet.'); setTimeout(() => setError(''), 4000); return }
    try {
      ...
      recorder.onstop = () => {
        stream.getTracks().forEach(t => t.stop()); clearInterval(recordingTimerRef.current); setIsRecording(false)
        const wasCancelled = recorder._cancelled; setRecordingSeconds(0)
      ...
      mediaRecorderRef.current = recorder; recorder.start(); setIsRecording(true); setRecordingSeconds(0)
      recordingTimerRef.current = setInterval(() => { setRecordingSeconds(prev => { if (prev + 1 >= MAX_RECORDING_SECONDS) { stopRecording(); return prev }; return prev + 1 }) }, 1000)
    } catch { setError('Could not access your microphone.'); setTimeout(() => setError(''), 4000) }
  }

  const stopRecording = () => { if (mediaRecorderRef.current && mediaRecorderRef.current.state !== 'inactive') mediaRecorderRef.current.stop() }
  const cancelRecording = () => { if (mediaRecorderRef.current && mediaRecorderRef.current.state !== 'inactive') { mediaRecorderRef.current._cancelled = true; mediaRecorderRef.current.stop() } }
  ...
  const updateSetting = async (key, value) => {
    ...
    catch { if (key==='files_enabled') setFilesEnabled(!value); if (key==='provider_files_need_approval') setProviderNeedsApproval(!value); if (key==='client_files_need_approval') setClientNeedsApproval(!value); if (key==='negotiation_mode') setNegotiationMode(!value); setError('Failed to update setting.'); setTimeout(() => setError(''), 3000) }
  }

  const toggleNegotiationMode = () => { const next = !negotiationMode; setNegotiationMode(next); updateSetting('negotiation_mode', next); if (next) setMessageTarget('client') }

  const inviteProvider = async () => {
    if (!selectedProvider) { setError('Select a provider first'); setTimeout(() => setError(''), 3000); return }
    try { const res = await api.post('/chat/rooms/' + activeRoom.id + '/invite-provider/', { provider_id: selectedProvider }); setActiveRoom(res.data); setSelectedProvider(''); setInviteMsg('Provider invited!'); setTimeout(() => setInviteMsg(''), 3000); api.get('/chat/rooms/').then(r => setRooms(r.data)) }
  ...
  const inviteClientByDropdown = async () => {
    ...
    try { const res = await api.post('/chat/rooms/' + activeRoom.id + '/invite-client/', { client_id: selectedClient }); setActiveRoom(res.data); setSelectedClient(''); setInviteClientMsg('✅ Client added!'); setTimeout(() => setInviteClientMsg(''), 3000); api.get('/chat/rooms/').then(r => setRooms(r.data)) }
  ...
  const inviteClientByPhone = async () => {
    ...
    try { const res = await api.post('/chat/rooms/' + activeRoom.id + '/invite-client/', { phone_number: invitePhone.trim() }); setActiveRoom(res.data); setInvitePhone(''); setInviteClientMsg('✅ Client invited!'); setTimeout(() => setInviteClientMsg(''), 3000) }
    catch (err) { setInviteClientMsg('🚫 ' + (err.response?.data?.error || 'Failed')); setTimeout(() => setInviteClientMsg(''), 4000) }
  }
  ...
  const roleStyle = role => role==='admin' ? {bg:C.adminBg,text:C.adminText} : role==='provider' ? {bg:C.providerBg,text:C.providerText} : {bg:C.clientBg,text:C.clientText}
  const avatarBg = name => { const colors=['#3b82f6','#8b5cf6','#ec4899','#f97316','#10b981','#06b6d4','#6366f1','#f59e0b']; let h=0; for(let i=0;i<(name||'').length;i++) h=name.charCodeAt(i)+((h<<5)-h); return colors[Math.abs(h)%colors.length] }
  ...
  const getRoomDisplayName = (room) => {
    ...
    if (isAdmin) { const clientId = room.client?.display_name || room.client?.client_id || ''; return clientId ? `${courseName} ${clientId}` : courseName }
    return courseName
  }

  const getProviderDisplayName = (name, index = 0) => { if (isAdmin || isProvider) return name; return `TutorJames${index + 1}` }
  const roleLabel = (role) => { if (!isAdmin && !isProvider) { if (role === 'provider') return 'Professor'; if (role === 'client') return 'Student' }; return role }
  const clientRealName = (clientObj) => { if (!clientObj) return 'Client'; if (clientObj.first_name || clientObj.last_name) return `${clientObj.first_name||''} ${clientObj.last_name||''}`.trim(); return clientObj.display_name || 'Client' }
  const getClientDisplay = (room) => { if (!room?.client) return 'Client'; if (isAdmin || isProvider) return room.client?.display_name || 'Client'; return clientRealName(room.client) }
  ...
  const allMembers = activeRoom ? (() => {
    ...
    if (!meIsListed) rows.unshift({ name: user?.display_name||'You', role: user?.role||'client', online: onlineUserIds.has(user?.id), id: user?.id })
    return rows.map(r => r.id === user?.id ? { ...r, isYou: true } : r)
  })() : []

  if (!activeRoom) return (
    <div style={S.loadingScreen}><div style={S.spinner}/><div style={S.loadingText}>Loading workspace…</div></div>
  )
  ...
  const handleDownloadClick = () => {
    if (window.deferredPrompt) {
      ...
      window.open('https://tutorjamesconnect.onrender.com', '_blank')
    }
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh' }}>
      ...
      {/* ── Permanent download banner ── */}
      <div style={S.downloadBanner} onClick={handleDownloadClick}>
        <span style={{ fontSize: 16 }}>📲</span>
        <span style={S.downloadBannerText}>Download TutorJamesConnect App</span>
      ...
      <div style={{ ...S.app, height: 'auto', flex: 1, minHeight: 0 }}>
        ...
        {/* Mobile overlays */}
        <div className={"tjc-overlay" + (showSidebar && window.innerWidth < 768 ? " show" : "")} onClick={() => setShowSidebar(false)} />
        <div className={"tjc-overlay" + (showRightPanel && window.innerWidth < 768 ? " show" : "")} onClick={() => setShowRightPanel(false)} />
        ...
        {showSidebar && (
          <aside className={"tjc-sidebar-mobile" + (showSidebar ? " show" : "")} style={S.sidebar}>
            <div style={S.sidebarBrand}>
              ...
              <div><div style={S.brandName}>TutorJamesConnect</div><div style={S.brandTagline}>Academic Excellence Platform</div></div>
            </div>
            <div style={S.userCard}>
              <div style={{ ...S.userAv, background: avatarBg(user?.display_name) }}>{user?.display_name?.[0]?.toUpperCase()||'U'}</div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={S.userName}>{sidebarDisplayName()}</div>
                <span style={{ ...S.rolePill, background: roleStyle(user?.role).bg, color: roleStyle(user?.role).text }}>{roleLabel(user?.role||'client')}</span>
              </div>
            </div>
            <div style={S.searchWrap}>
              ...
              <input style={S.searchInput} placeholder="Search rooms or members…" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
            </div>
            {isAdmin && (
              <div style={{ padding: '0 12px 12px' }}>
                <button className="tjc-newroom" style={S.newRoomBtn}><span style={{ fontSize: 16, lineHeight: 1 }}>+</span> New Room</button>
              </div>
            )}
            ...
            <div style={S.roomList}>
              {rooms.filter(r => !searchQuery || r.name.toLowerCase().includes(searchQuery.toLowerCase())).map(room => {
                ...
                return (
                  <div key={room.id} className="tjc-room" style={{ ...S.roomItem, ...(isActive ? S.roomItemActive : {}) }}
                    onClick={() => { setActiveRoom(room); setUnreadCounts(prev => ({ ...prev, [room.id]: 0 })); if (window.innerWidth < 768) setShowSidebar(false) }}>
                    <div style={{ ...S.roomAv, background: isActive ? C.gold : avatarBg(room.name), color: '#fff' }}>{room.name?.[0]?.toUpperCase()}</div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ ...S.roomName, color: isActive ? C.navy : C.text, fontWeight: isActive ? 700 : 600 }}>{getRoomDisplayName(room)}</div>
                      <div style={S.roomMeta}><span style={{ ...S.dot, background: statusColor(room.status) }} />{statusLabel(room.status)}</div>
                    </div>
        ...
        <div style={S.main}>
          <div style={S.header}>
            <div style={S.headerL}>
              ...
              <div style={{ ...S.roomHeaderAv, background: avatarBg(activeRoom.name) }}>{activeRoom.name?.[0]?.toUpperCase()}</div>
              <div>
                ...
                <div className="tjc-header-room-meta" style={S.headerRoomMeta}>
                  ...
                  {(isAdmin || isProvider) && (<><span style={S.sep}>·</span>{allMembers.length} members</>)}
                  <span style={S.sep}>·</span>Files {filesEnabled ? 'enabled' : 'disabled'}
                </div>
              </div>
            </div>
            <div style={S.headerR}>
              ...
              {pushSupported && (
                <button className="tjc-icon" style={{ ...S.iconBtn, color: pushSubscribed ? C.gold : S.iconBtn.color, opacity: pushBusy ? 0.5 : 1 }} onClick={togglePush} disabled={pushBusy}>
                  {pushSubscribed
          ...
          <div className="tjc-messages-area" style={S.messages}>
            {messages.length === 0 && connected && (
              <div style={S.emptyChat}><div style={S.emptyIcon}>💬</div><div style={S.emptyTitle}>No messages yet</div><div style={S.emptySub}>Start the conversation</div></div>
            )}
            {messages.map((msg, idx) => {
              if (msg.type === 'system') {
                if (!isAdmin && !isProvider) return null
                return <div key={msg.id||idx} style={S.sysMsg}><span style={S.sysText}>{msg.message}</span></div>
              }
              if (msg.type === 'file') {
                const isImg = isImageFile(msg.file_name); const isAudio = isAudioFile(msg.file_name)
                const isPending = msg.status === 'pending'; const isRejected = msg.status === 'rejected'
                return (
                  <div key={msg.id||idx} style={{ ...S.msgRow, justifyContent: 'flex-start' }}>
                    ...
                    <div style={{ maxWidth: '60%' }}>
                      <div style={S.msgMeta}>
                        <span style={S.msgSender}>{msg.role === 'provider' ? getProviderDisplayName(msg.sender, (activeRoom.providers||[]).findIndex(p => p.display_name === msg.sender)) : msg.sender}</span>
                        {msg.role && <span style={{ ...S.rolePill, background: roleStyle(msg.role).bg, color: roleStyle(msg.role).text }}>{roleLabel(msg.role)}</span>}
                      </div>
                      <div style={{ ...S.fileBubble, ...(isPending ? { borderColor: C.goldBorder, background: C.goldLight } : {}), ...(isRejected ? { borderColor: '#f5b5b5', background: C.redLight } : {}) }}>
                        ...
                        {!isAudio && (<div style={{ display: 'flex', alignItems: 'center', gap: 10 }}><span style={{ fontSize: 26 }}>{isImg ? '🖼️' : '📄'}</span><div><div style={S.fileName}>{msg.file_name}</div><div style={S.fileMeta}>{msg.file_size}</div></div></div>)}
                        ...
                        {isAudio && msg.file_url && !isRejected && (<div><div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}><span style={{ fontSize: 18 }}>🎤</span><span style={S.fileMeta}>{msg.file_size}</span></div><audio controls src={msg.file_url} style={{ width: 220, height: 34 }} /></div>)}
                        {!isAudio && msg.file_url && !isRejected && <a href={msg.file_url} target="_blank" rel="noreferrer" style={S.dlLink}>↓ Download</a>}
                        {isAdmin && isPending && (
                          <div style={{ display: 'flex', gap: 6, marginTop: 10 }}>
                            <button className="tjc-approve" style={S.appBtn} onClick={async () => { try { await api.post('/chat/admin/files/' + msg.file_id + '/approve/'); setMessages(p => p.map(m => m.id === msg.id ? { ...m, status: 'approved' } : m)); setPendingFiles(p => p.filter(f => f.id !== msg.file_id)) } catch { setError('Failed to approve.') } }}>✓ Approve</button>
                            <button className="tjc-reject" style={S.rejBtn} onClick={async () => { try { await api.post('/chat/admin/files/' + msg.file_id + '/reject/'); setMessages(p => p.map(m => m.id === msg.id ? { ...m, status: 'rejected' } : m)); setPendingFiles(p => p.filter(f => f.id !== msg.file_id)) } catch { setError('Failed to reject.') } }}>✕ Reject</button>
                          </div>
                        )}
                      </div>
                  ...
                  <div style={{ maxWidth: '62%' }}>
                    {!isMe && (
                      <div style={{ ...S.msgMeta, justifyContent: 'flex-start' }}>
                        <span style={S.msgSender}>{sRole === 'provider' ? getProviderDisplayName(msg.sender, (activeRoom.providers||[]).findIndex(p => p.display_name === msg.sender)) : msg.sender}</span>
                        <span style={{ ...S.rolePill, background: roleStyle(sRole).bg, color: roleStyle(sRole).text }}>{roleLabel(sRole)}</span>
                      </div>
                    )}
                    <div style={{ ...S.bubble, ...(isMe ? S.bubbleMe : S.bubbleOther), ...(isFlagged ? { background: C.amberLight, border: `1.5px solid ${C.goldBorder}`, boxShadow: 'none' } : {}) }}>
                      {isFlagged && (isAdmin || isProvider) && <div style={S.flagTag}>⚠️ Pending approval</div>}
                      {msg.reply_to && (<div style={S.quotedBox}><div style={S.quotedSender}>{msg.reply_to.sender}</div><div style={S.quotedBody}>{msg.reply_to.body}</div></div>)}
                      <span style={{ color: C.text }}>{msg.body}</span>
                      {isAdmin && isFlagged && (
                        <div style={{ display: 'flex', gap: 6, marginTop: 10 }}>
                          <button className="tjc-approve" style={S.appBtn} onClick={async () => { try { await api.post('/chat/admin/messages/' + msg.id + '/approve/'); setMessages(p => p.map(m => m.id === msg.id ? { ...m, status: 'approved' } : m)); setPendingMessages(p => p.filter(x => x.id !== msg.id)) } catch { setError('Failed to approve.') } }}>✓ Approve</button>
                          <button className="tjc-reject" style={S.rejBtn} onClick={async () => { try { await api.post('/chat/admin/messages/' + msg.id + '/reject/'); setMessages(p => p.filter(m => m.id !== msg.id)); setPendingMessages(p => p.filter(x => x.id !== msg.id)) } catch { setError('Failed to reject.') } }}>✕ Reject</button>
                        </div>
                      )}
                    </div>
          ...
          {activeRoom.status !== 'closed' && user?.role === 'client' && (
            <div style={S.inviteBar}>
              <input style={S.inviteInput} placeholder="Invite a friend — e.g. +965 XXXX XXXX" value={invitePhone} onChange={e => setInvitePhone(e.target.value)} onKeyDown={e => e.key === 'Enter' && inviteClientByPhone()} />
              <button style={S.inviteBtn} onClick={inviteClientByPhone}>Invite</button>
            </div>
          )}
          {inviteClientMsg && user?.role === 'client' && (
            <div style={{ textAlign: 'center', fontSize: 12, padding: '4px 20px', fontWeight: 500, color: inviteClientMsg.startsWith('✅') ? C.green : C.red, background: inviteClientMsg.startsWith('✅') ? C.greenLight : C.redLight }}>{inviteClientMsg}</div>
          )}
          ...
          <div className="tjc-input-area" style={S.inputArea}>
            {isRecording ? (
              ...
              <div style={S.inputBox}>
                ...
                <button className="tjc-icon" style={{ ...S.inputIco, opacity: filesEnabled && connected ? 1 : 0.3 }} onClick={() => filesEnabled && connected && fileInputRef.current?.click()}>
                ...
                <textarea style={S.textarea} placeholder={!connected ? 'Connecting…' : messageTarget === 'client' ? 'Message to client only…' : messageTarget === 'provider' ? 'Message to provider only…' : messageTarget === 'admin' ? 'Private message to admin…' : 'Type a message'} value={input} onChange={e => setInput(e.target.value)} onKeyDown={handleKeyDown} rows={1} disabled={!connected} />
                ...
                {user?.role === 'client' && !input.trim() && (
                  <button className="tjc-icon" style={{ ...S.inputIco, opacity: filesEnabled && connected ? 1 : 0.3 }} onClick={() => filesEnabled && connected && startRecording()}>
                ...
                <button className="tjc-send" style={{ ...S.sendBtn, ...(!(input.trim() && connected) ? S.sendOff : {}) }} onClick={sendMessage} disabled={!input.trim() || !connected}>
            ...
            {(isAdmin || isProvider) && activeRoom.status !== 'closed' && (
              <div className="tjc-target-row" style={S.targetRow}>
                ...
                {targetOptions.map(opt => (
                  <button key={opt.value} className="tjc-target" style={{ ...S.targetBtn, ...(messageTarget === opt.value ? S.targetOn : {}) }} onClick={() => setMessageTarget(opt.value)}>{opt.icon} {opt.label}</button>
                ))}
                {isAdmin && (<button className="tjc-target" onClick={toggleNegotiationMode} style={{ ...S.targetBtn, ...(negotiationMode ? S.negotiationOn : {}) }}>🤝 Negotiation {negotiationMode ? '• ON' : ''}</button>)}
          ...
          <aside className={"tjc-rightpanel-mobile" + (showRightPanel ? " show" : "")} style={S.rightPanel}>
            <div style={S.tabs}>
              {['members','roominfo'].map(t => (
                <button key={t} className="tjc-tab" style={{ ...S.tab, ...(rightTab === t ? S.tabOn : {}) }} onClick={() => setRightTab(t)}>
            ...
            <div style={{ flex: 1, overflowY: 'auto' }}>
              {rightTab === 'members' && (
                <>
                  <div style={S.rSec}>
                    <div style={S.rSecHdr}>MEMBERS ({allMembers.length}){isAdmin && <button style={S.addBtn} onClick={() => setRightTab('roominfo')}>+ Add</button>}</div>
                    {allMembers.map((m, i) => (
                      <div key={i} style={S.memberRow}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <div style={{ fontSize: 11, color: m.online ? C.green : C.textFaint, marginTop: 1 }}>{m.online ? '● Online' : '● Offline'}</div>
                        </div>
                        <span style={{ ...S.rolePill, background: roleStyle(m.role).bg, color: roleStyle(m.role).text }}>{roleLabel(m.role).charAt(0).toUpperCase() + roleLabel(m.role).slice(1)}</span>
                        {isAdmin && (m.isProvider || m.isExtraClient) && (
                          <button style={S.removeBtn} onClick={async () => { try { const ep = m.isProvider ? 'remove-provider' : 'remove-client'; const key = m.isProvider ? 'provider_id' : 'client_id'; const res = await api.post('/chat/rooms/' + activeRoom.id + '/' + ep + '/', { [key]: m.id }); setActiveRoom(res.data); api.get('/chat/rooms/').then(r => setRooms(r.data)) } catch { setError('Failed to remove.') } }}>
                    ...
                    <div style={S.rSec}>
                      ...
                      {[
                        ...
                        <div key={item.key} style={S.togRow}>
                          <div style={{ flex: 1 }}><div style={S.togLbl}>{item.label}</div><div style={S.togSub}>{item.sub}</div></div>
                          <div style={{ ...S.tog, background: item.val ? C.gold : C.border }} onClick={() => { const v = !item.val; item.set(v); updateSetting(item.key, v) }}>
                  ...
                  {isAdmin && (pendingMessages.length > 0 || pendingFiles.length > 0) && (
                    <div style={S.rSec}>
                      <div style={S.rSecHdr}>PENDING<span style={{ background: C.red, color: '#fff', fontSize: 9, fontWeight: 700, padding: '1px 6px', borderRadius: 10, marginLeft: 6 }}>{totalPending}</span></div>
                      {[...pendingMessages.map(p => ({ ...p, _type: 'msg' })), ...pendingFiles.map(f => ({ ...f, _type: 'file' }))].map(item => (
                        <div key={item.id} style={S.pendCard}>
                          ...
                          <div style={{ fontSize: 12, color: C.textMid, fontStyle: 'italic', marginBottom: 7 }}>{item._type === 'msg' ? `"${item.body || item.text}"` : `${isImageFile(item.file_name) ? '🖼️' : '📄'} ${item.file_name}`}</div>
                          {item._type === 'file' && (item.file_url || item.file) && (
                            <div style={{ marginBottom: 8 }}>
                              {isImageFile(item.file_name) && <img src={item.file_url||item.file} alt={item.file_name} style={{ width: '100%', borderRadius: 6, marginBottom: 6, maxHeight: 140, objectFit: 'cover' }} />}
                              <a href={item.file_url||item.file} target="_blank" rel="noreferrer" style={{ ...S.dlLink, marginTop: 0, fontSize: 11 }}>👁 Preview / Download</a>
                            </div>
                          )}
                          <div style={{ display: 'flex', gap: 6 }}>
                            <button className="tjc-approve" style={S.appBtn} onClick={async () => { try { if (item._type==='msg') { await api.post('/chat/admin/messages/'+item.id+'/approve/'); setPendingMessages(p=>p.filter(x=>x.id!==item.id)); setMessages(p=>p.map(m=>m.id===item.id?{...m,status:'approved'}:m)) } else { await api.post('/chat/admin/files/'+item.id+'/approve/'); setPendingFiles(p=>p.filter(x=>x.id!==item.id)); setMessages(p=>p.map(m=>m.id==='file_pending_'+item.id?{...m,status:'approved'}:m)) } } catch { setError('Failed to approve.') } }}>✓ Approve</button>
                            <button className="tjc-reject" style={S.rejBtn} onClick={async () => { try { if (item._type==='msg') { await api.post('/chat/admin/messages/'+item.id+'/reject/'); setPendingMessages(p=>p.filter(x=>x.id!==item.id)); setMessages(p=>p.filter(m=>m.id!==item.id)) } else { await api.post('/chat/admin/files/'+item.id+'/reject/'); setPendingFiles(p=>p.filter(x=>x.id!==item.id)); setMessages(p=>p.filter(m=>m.id!=='file_pending_'+item.id)) } } catch { setError('Failed to reject.') } }}>✕ Reject</button>
                          </div>
                        </div>
                      ))}
                    </div>
              ...
              {rightTab === 'roominfo' && isAdmin && (
                <div style={S.rSec}>
                  ...
                  <div style={S.togRow}>
                    ...
                    <div style={{ ...S.tog, background: soundEnabled ? C.gold : C.border }} onClick={toggleSound}><div style={{ ...S.togKnob, transform: soundEnabled ? 'translateX(18px)' : 'translateX(2px)' }} /></div>
                  </div>
                  {soundEnabled && SOUND_PROFILES.map(profile => (
                    <div key={profile.id} style={{ ...S.soundOpt, ...(messageSoundProfile === profile.id ? { background: C.adminBg, border: `1.5px solid ${C.gold}` } : {}) }} onClick={() => changeMessageSoundProfile(profile.id)}>
...
const S = {
  ...
  downloadBanner: { display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, background: '#075e54', color: '#ffffff', padding: '8px 16px', fontSize: 13, fontWeight: 600, cursor: 'pointer', flexShrink: 0, letterSpacing: '0.01em', userSelect: 'none', zIndex: 100 },
  downloadBannerText: { fontSize: 13, fontWeight: 600 },
